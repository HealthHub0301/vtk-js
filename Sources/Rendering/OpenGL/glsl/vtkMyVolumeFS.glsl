  ...
  if (float(tColor.a) > 0.01) {
    for (int i = -outlineThickness; i <= outlineThickness; i++) {
      for (int j = -outlineThickness; j <= outlineThickness; j++) {
        ...
        vec4 neighborPosIS = fragCoordToIndexSpace(neighborPixelCoord);
        vec4 value = getTextureValue(neighborPosIS);
...
// method for apply windowing on volume data
...
void applyBlend(vec3 posIS, vec3 endIS, float sampleDistanceIS, vec3 tdims)
{
  ...
  tValue = getTextureValue(posIS);
    ...
    gl_FragData[0] = getColorForValue(sum, posIS, tstep);
  #endif
  // vtk에서 제공하는 blendmode가 하나 추가됨에 따라 우리 것도 하나씩 순서가 밀리는 걸로..
    ...
    for (int i = 0; i < //VTK::MaximumSamplesValue ; ++i)
    {
      ...
      // now map trough opacity and color
      // preintegration
  ...
  #if vtkBlendMode == 6 // custom blend mode gradient opacity
    // now map through opacity and color
    tColor = getColorForValue(tValue, posIS, tstep);
    ...
    vec2 st = gl_FragCoord.xy / vec2(300, 300); // 수정 필요
